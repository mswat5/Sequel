...
const MENU = [
	...
	{
		title: "Labs",
		subtitle: "Index & analyze your reports",
		path: "/sheets",
		icon: SettingsIcon,
	},
	{
	...
	{
		title: "Tests",
		subtitle: "Optimize your health",
		path: "/tests",
		icon: LogoIcon,
	},
	{
		title: "Therapies",
		subtitle: "Get the best care",
		path: "/therapies",
		icon: ScratchPad,
	}
];
...
const Sidebar = () => {
	...
	return (
		<div className="max-w-[25%] w-[400px] bg-[rgba(0,0,0,0.30)] rounded-3xl h-[calc(100vh-64px)] pt-2 flex flex-col shadow-[0px_2px_14px_0px_rgba(0,0,0,0.05)]">
			<header className="flex flex-col gap-8 mr-6 my-2 mb-0 items-end">
				<svg
			...
			<div className="flex-1 mt-4 relative flex flex-col gap-2 overflow-y-auto">
				{MENU.map((item) => (
					<div key={item.title} className="relative">
						<Link
							to={item.path}
							className={classNames(
								"flex items-center mx-4 px-4 py-4 cursor-pointer hover:bg-[rgba(0,0,0,0.50)] rounded-xl",
								{
									"bg-[rgba(0,0,0,0.50)]": location.pathname.includes(
										item.path
									),
								}
							)}
						>
							<img src={item.icon} className="h-16 w-16"></img>
							<div className="flex flex-col items-start ml-4 flex-1">
								<div className="text-lg font-semibold text-[#fff]">
									{item.title}
								</div>
								<div className="text-xs text-[#ACACB7] text-start">
									{item.subtitle}
								</div>
							</div>
						</Link>
